

const fr = {
  new_password: 'Enter new password',
  new_pass: 'New Password',
  confirm_new_password: 'Confirm New Password',
  change_password: 'Change Password',
  invalid_link: 'Invalid Link',
  invalid_reset_link: 'Invalid Password Reset Link. Redirected to Login Page',
  reset_password_failed: 'Reset Password Failed, Kindly retry again',
  password_failed: 'Password Failed',
  password_reset_failed: 'Password Reset Failed',
  password_reset_failed_text: 'Password Reset failed. Please try again',
  login_attempt_failed: 'Login Attempt failed',
  account_deactivated: 'Account deactivated',
  password_reset_successful: 'Password Reset Successful',
  password_reset_successful_text: 'Password Reset Successfull. You will automatically be logged in',
  login_failed: 'Login failed',
  login_failed_text: 'Login failed. Please try again',
  password_not_match: 'Password does not match. Please try again',
};


export default fr;
