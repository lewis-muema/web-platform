      ...
      this.$store.dispatch('$_freight/requestApproveApprover', fullPayload).then(
        (response) => {
          ...
          if (workingResponse.status) {
            this.doNotification(
              1,
              this.$t('settingsComponent.approver_updated'),
              this.$t('settingsComponent.approver_removed'),
            );
            this.fetchApproversList();
          } else {
    ...
    submitApproval() {
      if (this.approver !== '') {
          ...
          (response) => {
            ...
            if (workingResponse.status) {
              this.doNotification(
                1,
                this.$t('settingsComponent.approver_updated'),
                this.$t('settingsComponent.approver_updated_successfully'),
              );
              this.fetchApproversList();
            } else {
        ...
        this.doNotification(
          2,
          this.$t('settingsComponent.failed_add_approver'),
          this.$t('settingsComponent.failed_add_approver_single'),
        );
      }
    },
